...
Повторна заливка потрібна лише коли контентники принесуть новий пакет.
...
### Автоматична приймальня (без участі розробника)

Щоб нові партії не залежали від локальної машини, на сервері є приймальня, у
яку контентники кладуть архів вигрузки по SFTP. Далі все відбувається саме.

Що бачить контентник:

```
хост:   162.55.40.48
логін:  blogdrop
доступ: тільки SFTP, тільки каталог /upload
```

Акаунт замкнений у `/var/lib/blog-intake` (`ChrootDirectory` +
`ForceCommand internal-sftp`), звичайний SSH для нього заборонений. Блок
`Match User blogdrop` стоїть **у самому кінці** `/etc/ssh/sshd_config`: там
`Include` на 12-му рядку, тому `Match` у `sshd_config.d/` загнав би у свій
контекст решту конфігу разом із `Subsystem sftp`, і sshd не запустився б.
